...
//                is TmpL.Test -> TODO()
    ...
    private fun translateStatement(statement: TmpL.Statement): List<Rust.Statement> {
        try {
            return when (statement) {
                ...
                is TmpL.TryStatement -> return translateTryStatement(statement)
                ...
                is TmpL.ThrowStatement -> Rust.ExprStatement(
                    statement.pos,
                    "panic!".toId(statement.pos).call(listOf(Rust.StringLiteral(statement.pos, "bubble"))),
                )
                is TmpL.YieldStatement -> Rust.ExprStatement(
                    statement.pos,
                    "yield".toKeyId(statement.pos).call(listOf("()".toId(statement.pos))),
                )
            }.let { listOf(it) }
    ...
    private fun translateTryStatement(statement: TmpL.TryStatement): List<Rust.Statement> {
        // TryStatement is generated by CatchBubble strategy, which Rust doesn't use
        // (Rust uses IfHandlerScopeVar). Handle gracefully by inlining both blocks.
        val tried = translateStatement(statement.tried)
        val recover = translateStatement(statement.recover)
        return tried + recover
    }
