...
object BuiltinFuns {
    ...
    val ltIntFn: CallableValue = IntCompareFun("<") { cmp ->
        TBoolean.value(cmp < 0)
    }
    val vLtIntFn = Value(ltIntFn)

    val leIntFn: CallableValue = IntCompareFun("<=") { cmp ->
        TBoolean.value(cmp <= 0)
    }
    val vLeIntFn = Value(leIntFn)

    val gtIntFn: CallableValue = IntCompareFun(">") { cmp ->
        TBoolean.value(cmp > 0)
    }
    val vGtIntFn = Value(gtIntFn)

    val geIntFn: CallableValue = IntCompareFun(">=") { cmp ->
        TBoolean.value(cmp >= 0)
    }
