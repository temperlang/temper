...
import lang.temper.type.WellKnownTypes.functionType2
import lang.temper.type.WellKnownTypes.invalidType2
import lang.temper.type.WellKnownTypes.typeType2
...
import lang.temper.type2.InternalTyperFailureException
import lang.temper.type2.MkType2
import lang.temper.type2.OutputBound
...
internal class Typer(
    ...
    private class Decision private constructor(
        // This class allows storing either old or new style type info
        // and retrieving either old or new style type info, as an
        ...
        val type2: Type2 get() = types.second.value
        ...
        override fun toString(): String = buildString {
            ...
            val (t1, t2) = types
            append(if (t1.isInitialized()) { "($t1, " } else { "(???, " })
            append(if (t2.isInitialized()) { "$t2)" } else { "???)" })
            if (variants != null) {
                val (v1, v2) = variants
                append(", variant=")
                append(if (v1.isInitialized()) { "($v1, " } else { "(???, " })
                append(if (v2.isInitialized()) { "$v2)" } else { "???)" })
            }
            if (bindingMaps != null) {
                val (b1, b2) = bindingMaps
                append(", bindings=")
                append(if (b1.isInitialized()) { "($b1, " } else { "(???, " })
                append(if (b2.isInitialized()) { "$b2)" } else { "???)" })
            }
        ...
        fun copy(type: Type2) = Decision(
            types = Pair(lazy { hackMapNewStyleToOld(type) }, lazyOf(type)),
            variants = variants,
            bindingMaps = bindingMaps,
            explanations = explanations,
        )

        fun copy(explanations: List<TypeReasonElement>) = Decision(
            types = types,
            variants = variants,
            bindingMaps = bindingMaps,
            explanations = explanations,
        ...
        constructor(
            ...
            variants = variant?.let { Pair(lazyOf(it), lazy { hackTryStaticTypeToSig(it)!! }) },
            bindingMaps = bindings?.let { Pair(lazyOf(it), lazy { hackMapOldStyleActualsToNew(it) }) },
            explanations = explanations,
        )

        constructor(
            type: Type2,
            variant: Signature2? = null,
            bindings: Map<TypeFormal, Type2>? = null,
            explanations: List<TypeReasonElement> = listOf(),
        ) : this(
            types = Pair(lazy { hackMapNewStyleToOld(type) }, lazyOf(type)),
            variants = variant?.let { Pair(lazy { typeFromSignature(it) }, lazyOf(it)) },
            bindingMaps = bindings?.let {
                Pair(
                    lazy { it.mapValues { (_, v) -> hackMapNewStyleToOld(v) } },
                    lazyOf(it),
                )
            },
            explanations = explanations,
    ...
    private inner class TypingInfo(val typerPlan: TyperPlan) {
        ...
        fun decisionType2(tree: Tree?) = decisions[tree]?.type2
            ...
            .forEachContinuing { t ->
                ...
                when (t) {
                    ...
                    is BasicTypeInferencesTree -> if (decision != null) {
                        t.typeInferences = BasicTypeInferences(decision.type2, decision.explanations)
                    } else if (t.needsTypeInfo && !t.isMetadataValue) {
                        val (e, type) = invalid2BecauseMissingType(t, "expr")
                    ...
                    is CallTypeInferencesTree -> if (decision != null) {
                        t.typeInferences = CallTypeInferences(
                            decision.type2,
                            decision.variant2 ?: invalidSig,
                            decision.bindings2 ?: emptyMap(),
    ...
    private fun typeRegularCall(tree: CallTree) {
        ...
        var effectiveCallee = if (isNewCall) {
            tree.child(1)
        } else {
            tree.childOrNull(0)
        }
        ...
        val explicitActualTypesAndPositions: List<Pair<StaticType, Position>>? =
            ...
            } else if (isNewCall && effectiveCallee != null) {
        ...
        fun specialize(sig: Signature2): Signature2 {
            val explicitActualTypesAndPositions = explicitActualTypesAndPositions
            return if (explicitActualTypesAndPositions == null) {
                sig
            } else {
                val bindings = buildMap {
                    for (i in sig.typeFormals.indices) {
                        val tf = sig.typeFormals[i]
                        val (actual) = explicitActualTypesAndPositions.getOrNull(i) ?: break
                        this[tf] = hackMapOldStyleToNew(actual)
                    }
                }
                sig.copy(typeFormals = sig.typeFormals.drop(bindings.size)).mapType(bindings)
            }
        }
        ...
        while (true) {
            ...
            if (ancestor is CallTree) {
                if (isAssignment(ancestor) && incoming == ancestor.edge(2)) {
                    // incoming is the right-hand side.
    ...
    private fun typeCallByParts2(
        ...
        var boundsRequireFeedback = false
        ...
        val outputBounds = buildList<OutputBound> outputBounds@{
            if (isNewCall) {
                val typeEdge = callTree.edge(1)
                val typeTree = typeEdge.target
                val reifiedType = typeTree.reifiedTypeContained
                    ?: return@outputBounds
                val constructedType = reifiedType.type2
                val typeFormals = constructedType.definition.formals
                val typeActuals = constructedType.bindings
                if (typeFormals.size > typeActuals.size) {
                    // A partial type
                    boundsRequireFeedback = true
                    add(
                        InputBound.IncompleteReification(
                            ...
                            reifiedType = reifiedType,
                            typeArgumentIndex = -1,
                            typeVar = solverVarNamer.unusedTypeVar("constructedType"),
                            reificationEdge = typeEdge,
                            describedValueArgumentIndex = null,
                        ),
                    )
                } else if (typeFormals.isNotEmpty() && typeFormals.size == typeActuals.size) {
                    add(InputBound.Pretyped(constructedType.at(typeTree.pos)))
                }
            }
        }
        if (!boundsRequireFeedback && isNewCall) {
            val typeConstructed = callTree.childOrNull(1)?.reifiedTypeContained?.type2
            if (typeConstructed != null && typeConstructed.bindings.size < typeConstructed.definition.formals.size) {
                // An incomplete type that needs completion, like `new Pair(k, v)` instead of `new Pair<K, V>(k, v)`.
                boundsRequireFeedback = true
            }
        }
        ...
        if (
            ...
            ti.decide(
                callTree,
                Decision(
                    type = excludeBubble(variant.sig.returnType2),
                    variant = variant.sig,
            ...
            for ((i, inputTree) in inputTrees.withIndex()) {
                if (inputTree is ValueLeaf && ti.isUndecided(inputTree)) {
                    ...
                    ti.decide(
                        inputTree,
                        if (formal != null) {
                            Decision(formal.type)
                        } else {
                            Decision(invalidType2, explanations = listOf(becauseRedundantArgument(inputTree)))
                        },
                    )
                }
            }
            ...
            var internalError: InternalTyperFailureException? = null
            try {
                inferBounds(typeContext, typeContext2, solverVarNamer, calls, debug = DEBUG)
            } catch (e: InternalTyperFailureException) {
                internalError = e
            }

            if (internalError != null) {
                val first = calls.first().callPosition
                val rest = calls.subList(1, calls.size).map { it.callPosition }
                val problem = LogEntry(
                    Log.Error,
                    MessageTemplate.InternalErrorTyper,
                    first,
                    listOf(internalError.constraintDump, rest),
                )
                for (call in calls) {
                    val callSite = call.destination
                    val allExplanations = buildList {
                        call.explanations?.let { addAll(it) }
                        add(TypeReason(problem))
                    }
                    val decision = Decision(
                        type = call.passType ?: invalidType2,
                        variant = invalidSig,
            ...
            for (call in calls) {
                ...
                if (chosenCallee != null && call.inputTrees.any { it is FunTree && ti.isUndecided(it) }) {
                    val bindings2 = (call.bindings ?: emptyMap())
                ...
                if (!isNewCall(callSite)) {
                    val variantType = AdHocArrowTypes.definedTypeForSig(decision.variant2 ?: invalidSig)
                    val callee = callSite.child(0)
                    val storedCalleeType = ti.decisionType2(callee)
    ...
    private fun fixupAssignmentCallee(tree: CallTree) {
        val callee = tree.child(0)
        val calleeType = ti.decisionType2(callee)
        if (calleeType == null || calleeType == functionType2) {
            val variant = ti.decision(tree)?.variant2
            if (variant != null) {
                ti.decide(callee, Decision(AdHocArrowTypes.definedTypeForSig(variant)))
            }
        }
    }
                ...
                ?: run {
                    explanations.add(
                        BecauseTypeInfoMissing(
                            ...
                            missingInfo = constructorShape.name as ResolvedName,
                        ),
                    )
                    invalidSig
                }
        ...
        val resultType: StaticType = run computeResultType@{
            if (parts == null) {
                ...
                val previousFunctionType = ti.decisionType(t) as? FunctionType
                    ...
                    ?: t.typeInferences?.type?.let { type ->
                        AdHocArrowTypes.reverseToSig(type)?.let { typeFromSignature(it) }
                    }
    ...
    private fun typeForNonLocalName(name: ResolvedName): Type2? = when (name) {
        is BuiltinName -> {
            ...
            if (reifiedType != null) {
                ...
                if (result is Value<*>) {
                    ...
                    val valueType = type2ForValue(result)
                    when (valueType) {
                        functionType2 -> {
                ...
                if (t is NameLeaf && ti.isUndecided(t)) {
                    ...
                    ti.decide(
                        ...
                        type?.let { Decision(it) }
                            ?: Decision(
                                InvalidType,
                                explanations = listOf(BecauseTypeInfoMissing(t.pos, name)),
                            ),
                    )
                    ...
                    if (right is ValueLeaf) {
                        ...
                        if (copyTypeRightward) {
                            val leftType = ti.decisionType2(left)
                    ...
                    val leftType = ti.decisionType2(left)
                    val rightType = ti.decisionType2(right)?.let { excludeBubble(it) }
                    if (
                        ti.decisionType(t) == null && leftType != null && rightType != null &&
                        typeContext2.isSubType(rightType, leftType) && !leftType.mentionsInvalid
                    ) {
                    ...
                    if (
                        type == null || !type.mentionsInvalid && (
                            leftType == null || rightType == null ||
                                !typeContext2.isSubType(rightType, leftType)
                            )
                    ) {
                        val rightTypeOrInvalid = rightType ?: invalidType2
                        val contradictions =
                            (ti.decision(t)?.explanations ?: emptyList()) +
                                BecauseIllegalAssignment(
                                    t.pos,
                                    leftType ?: invalidType2,
                                    rightTypeOrInvalid,
                                )
                        if (DEBUG) {
                        ...
                        val decisionWithError = ti.decision(t)?.let {
                            ...
                            invalidType2,
                            variant = Signature2(
                                returnType2 = rightTypeOrInvalid,
                                hasThisFormal = false,
                                requiredInputTypes = listOf(rightTypeOrInvalid, rightTypeOrInvalid),
                            ),
                            bindings = emptyMap(),
    ...
    private fun fixupCalleeType(t: CallTree): ITypedVariants? {
        ...
        if (variants !is SigVariants || variants.reasons.isNotEmpty()) {
            val fixedType = if (variants.fnTypes.size == 1) {
                variants.fnTypes[0]
            } else {
                functionType
            }
        ...
        console.groupIf(DEBUG, "typeForDotHelper") {
            ...
            dotHelper.extensions.forEach { extensionResolution ->
                ...
                if (extensionType != null) {
                    ...
                    explanations.add(BecauseTypeInfoMissing(t.pos, extensionResolution))
                    includeInvalid = true
                }
            }
    ...
    private fun typeForExtensionResolution(
        ...
        val extensionRef = extensionResolution.toLeaf(document, pos)
    ...
    companion object {
        ...
        private fun assignmentDecision(leftType: Type2) = Decision(
            leftType,
            variant = Signature2(
                returnType2 = leftType,
                hasThisFormal = false,
                requiredInputTypes = listOf(leftType, leftType),
            ),
        )
    }
...
private val StaticType.fnTypesIterable: Iterable<FunctionType> get() {
...
private fun MutableCollection<Callee>.explodeCalleeType(
    t: StaticType,
    priority: CalleePriority,
    specialize: (Signature2) -> Signature2,
) {
    when (t) {
        ...
        is NominalType -> {
            if (functionalInterfaceSymbol in t.definition.metadata) {
                withType(
                    ...
                    fn = { _, sig, _ ->
                        add(Callee(specialize(sig), priority))
                    },
                    fallback = {},
                )
            }
        ...
        is AndType ->
            for (mt in t.members) {
                explodeCalleeType(mt, priority, specialize)
            }
        OrType.emptyOrType -> {}
