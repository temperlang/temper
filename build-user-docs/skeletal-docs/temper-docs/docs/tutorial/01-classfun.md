...
future (see [issue#50]). Temper also enables rich behavior with both class-level
...
redefine our `Rectangle` type and add methods to it (see [issue#22] on
automating `.toString()`):
...
[issue#34] about the `'Any0'` type), but the `translate` helper function is
