...
see [issue#26] for reorganizing `std`):
...
constant regex patterns during compilation to each backend (see [issue#11]).
...
See [temper/**/regex.temper.md] in the Temper source for the full current schema
and features. We plan to extend this in the future (see [issue#9], [issue#16],
and more). Also see [temper/**/regex/match/match.temper.md] for more examples of
using the features.
...
[issue#11]). You should even be able to use pure functions to programmatically
...
[issue#9]). This is at the planning stage only, but we're hopeful we can work
