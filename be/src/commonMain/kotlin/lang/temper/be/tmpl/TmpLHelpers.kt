...
fun TmpL.Tree.mutableCaptures(): Set<ResolvedName> {
    ...
    val vars = buildSet {
        for (kid in children) {
            addAll(kid.varsDeclared(includeBlocks = true, includeNesting = true) { !it.assignOnce })
        }
    }
...
fun TmpL.Tree.varsDeclared(
    ...
    for (kid in children) {
        ...
        yieldAll(kid.varsDeclared(includeBlocks = includeNesting, includeNesting = includeNesting, keep = keep))
    }
}
