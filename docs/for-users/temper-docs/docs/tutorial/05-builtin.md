...
in the file [*Implicits.temper*](https://github.com/temperlang/temper/blob/main/frontend/src/commonMain/resources/implicits/Implicits.temper) in the Temper source code. The
...
See also [additional discussion and a diagram](../reference/types.md#type-relationships) of
some of these types:
...
rounding (see [issue#27](https://github.com/temperlang/temper/issues/27)).
...
- `Listed` - An interface for read-only access to data. Extended by both `List`
  ...
  extend it (see [issue#28](https://github.com/temperlang/temper/issues/28)), but that doesn't work correctly today.
...
user-defined key types in the future (see [issue#29](https://github.com/temperlang/temper/issues/29)).


## Other types

Temper has additional built-in types, but above discussion highlights some of the
most important ones. Also, some types currently in [*Implicits.temper*](https://github.com/temperlang/temper/blob/main/frontend/src/commonMain/resources/implicits/Implicits.temper),
such as `Deque<T>` and `DenseBitVector` might move to support libraries in the
future (see [issue#30](https://github.com/temperlang/temper/issues/30)).
...
reorgnization before 1.0 (see [issue#26](https://github.com/temperlang/temper/issues/26)). We already saw a bit of testing
